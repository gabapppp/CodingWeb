import docker
import os
from config import DOCKER_IMAGE, AUTO_REMOVE, FILE_OPEN_MODE, LOCAL_DIR, CONTAINER_DIR


def shell_runner(client, file_name, container_name, command, return_dict):
    try:
        shell_run_command = command + ' ' + str(file_name)
        ...
        result = client.containers.run(DOCKER_IMAGE, shell_run_command,
                                       remove=AUTO_REMOVE, 
                                       name=container_name,
